/*
Exercise 1:
Write a function testNum that takes a number as an argument and returns a Promise that tests if the value is less 
than or greater than the value 10. Log the result to the console.
*/

// const testNum = (num) => {
//   return new Promise((resolve, reject) => {
//     if (num > 10) {
//       resolve(`The number ${num} is greater than 10`);
//     } else {
//       reject(`The number ${num} is smaller dan 10`);
//     }
//   })
// };

// testNum(110).then(resolved => {console.log(resolved)})
//   .catch(rejected => {console.log(rejected)})

/*
Exercise 2:
Write two functions that use Promises that you can chain! The first function, makeAllCaps(), 
will take in an array of words and capitalize them, and then the second function, sortWords(), 
will sort the words in alphabetical order. If the array contains anything but strings, it should throw an error.
Then call these functions by *chaining* the promises
*/

const makeAllCaps = (words) => {
  return new Promise((resolve, reject) => {
    if (
      words.every((word) => {
        return typeof word === "string";
      })
    ) {
      resolve(
        sortWords(
          words.map((word) => {
            return word.toUpperCase();
          })
        )
      );
    } else {
      reject("this is not a string!");
    }
  });
};
const sortWords = (array) => {
  return array.sort();
};

const arrayOfWords = ["cucumber", "tomatos", "avocado"];
const complicatedArray = ["cucumber", 44, true];

makeAllCaps(arrayOfWords)
  .then(sortWords(arrayOfWords)) //het callen van de sortWords function
  .then((resolved) => {
    console.log(resolved);
  })
  .catch((rejected) => {
    console.log(rejected);
  });

makeAllCaps(complicatedArray)
  .then(sortWords(complicatedArray)) //het callen van de sortWords function
  .then((resolved) => {
    console.log(resolved);
  })
  .catch((rejected) => {
    console.log(rejected);
  });

// call both functions, chain them together and log the result to the console
